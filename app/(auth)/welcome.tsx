...
import { MOON_FRAME_BASE_URL, MOON_FRAME_START, MOON_FRAME_END } from '../../lib/constants';
...
function Star({ x, y, size, baseOpacity, duration }: {
  x: number; y: number; size: number; baseOpacity: number; duration: number;
}) {
      ...
      <View className="items-center px-8">
        ...
        >
          <Image
            ...
            placeholder={{ uri: `${MOON_FRAME_BASE_URL}/moon.${String(MOON_FRAME_START).padStart(4, '0')}.webp` }}
          />
        </View>

        {/* Branding */}
        <Text
          className="text-text-primary font-josefin-semibold mt-8"
        ...
        {/* Tagline */}
        <Text className="text-text-secondary text-sm font-josefin mt-2">
          live in tune with the sky
        </Text>

        {/* CTA section -- 48px gap from tagline */}
        <View className="w-full mt-12">
          {/* Begin button -- routes to Sign Up */}
          <Pressable
            onPress={() => router.push('/(auth)/sign-up')}
            className="h-[52px] bg-accent rounded-xl items-center justify-center"
